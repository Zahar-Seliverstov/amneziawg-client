...
Клиент управляет сетью с правами администратора, поэтому его API закрыт
токеном: петлевой интерфейс общий для всех, кто вошёл в систему, и без токена
конфигурации вместе с приватными ключами мог бы прочитать любой процесс любого
пользователя машины.

Токен рождается заново при каждом запуске и лежит в
`~/.config/awg-client/token` с правами `0600`. Приложение и `start.sh`
подставляют его сами; ссылка с токеном печатается в журнале при запуске.
Вручную:

```sh
curl -H "Authorization: Bearer $(cat ~/.config/awg-client/token)" \
     http://127.0.0.1:8081/api/vpn/status
```

Браузер обменивает токен на cookie при первом открытии ссылки, поэтому из
адресной строки он сразу исчезает.
