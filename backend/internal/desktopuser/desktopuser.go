...
//
// Службу поднимают через pkexec, поэтому сама она работает от root, а файлы
// создаёт для пользователя: сокет API и ярлык автозапуска. Оставить их
// принадлежащими root значит сделать их недоступными из обычной сессии — то
// есть сломать ровно то, ради чего их и создавали.
package desktopuser
