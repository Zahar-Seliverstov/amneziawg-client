...
warn_running() {
    ...
    # start.sh держит порт 8081, и оболочка прицепится к его backend'у
    # вместо того, чтобы поднять свой.
    if pgrep -f "start\.sh" >/dev/null 2>&1; then
        log_warn "Запущен ./start.sh — останови его перед запуском приложения (занят порт 8081)"
    fi
}
