...
// Цвета у групп нет намеренно: цветом в списке помечен вид правила (адрес,
// подсеть, домен, зона), и второй цветовой признак рядом читался бы как
// путаница. Группу держат заголовок и полоса слева.
...
export function useRuleGroups(
  ...
  const groups = computed<RuleGroup[]>(() => {
    ...
    // Порядок — по первому появлению в списке, а не по алфавиту: так группы
    // не перетасовываются при каждом добавлении правила.
    return [...found.entries()]
      .filter(([, list]) => list.length >= 2)
      .map(([source, list]) => ({ source, rules: list }))
  })
